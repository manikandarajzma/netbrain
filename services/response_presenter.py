...
def build_troubleshoot_content(
    ...
    interface_counters = group_interface_counters(session_data.get("interface_counters", []))
    ...
    if final_text and servicenow_summary:
        final_text = replace_markdown_section(final_text, "ServiceNow", servicenow_summary)
...
def build_network_ops_content(
    ...
    path_hops = session_data.get("path_hops", [])
    reverse_path_hops = session_data.get("reverse_path_hops", [])
    include_path = should_include_network_ops_path(prompt)
