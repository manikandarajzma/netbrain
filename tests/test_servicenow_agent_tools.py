...
class ServiceNowAgentToolContractTests(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("tools.servicenow_agent_tools.push_status", new_callable=AsyncMock)
    @patch("tools.servicenow_agent_tools.pending_approval_store.set")
    @patch("tools.servicenow_agent_tools.call_mcp_tool", new_callable=AsyncMock)
    async def test_create_servicenow_incident_stages_pending_approval_by_default(
        self,
        mock_call_mcp_tool,
        mock_pending_set,
        _mock_push_status,
        ...
        result = await create_servicenow_incident.coroutine(
            ...
            ci_name="arista-ai1",
            config={"configurable": {"session_id": "tool-test", "request_prompt": "create an incident"}},
        )

        self.assertIn("Proposed action: Create ServiceNow incident", result)
        mock_pending_set.assert_called_once()
        mock_call_mcp_tool.assert_not_awaited()

    @patch("tools.servicenow_agent_tools.push_status", new_callable=AsyncMock)
    @patch("tools.servicenow_agent_tools.call_mcp_tool", new_callable=AsyncMock)
    async def test_create_servicenow_incident_execute_mode_uses_standard_unexpected_response_contract(self, mock_call_mcp_tool, _mock_push_status):
        mock_call_mcp_tool.return_value = "bad-result"

        result = await create_servicenow_incident.coroutine(
            short_description="Connectivity issue",
            config={"configurable": {"session_id": "tool-test", "approval_mode": "execute"}},
        )
    ...
    @patch("tools.servicenow_agent_tools.pending_approval_store.set")
    @patch("tools.servicenow_agent_tools.call_mcp_tool", new_callable=AsyncMock)
    async def test_create_servicenow_change_request_stages_pending_approval_by_default(
        self,
        mock_call_mcp_tool,
        mock_pending_set,
        _mock_push_status,
    ):
        result = await create_servicenow_change_request.coroutine(
            short_description="Route map update",
            ci_name="arista-ai1",
            justification="Restore routing",
            implementation_plan="Apply route map",
            config={"configurable": {"session_id": "tool-test", "request_prompt": "create a change"}},
        )

        self.assertIn("Proposed action: Create ServiceNow change request", result)
        mock_pending_set.assert_called_once()
        mock_call_mcp_tool.assert_not_awaited()

    @patch("tools.servicenow_agent_tools.push_status", new_callable=AsyncMock)
    @patch("tools.servicenow_agent_tools.call_mcp_tool", new_callable=AsyncMock)
    async def test_update_change_request_execute_mode_uses_standard_failed_contract(self, mock_call_mcp_tool, _mock_push_status):
