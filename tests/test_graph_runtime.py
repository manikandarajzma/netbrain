...
class GraphRuntimeHelperTests(unittest.TestCase):
    def test_build_initial_state_sets_expected_defaults(self):
        state = build_initial_state("hello", [{"role": "user", "content": "prev"}], "alice", "session-1")
    ...
    def test_build_graph_config_includes_thread_id_only_when_present(self):
        self.assertEqual(build_graph_config(None), {"recursion_limit": 50})
        self.assertEqual(
            build_graph_config("session-1"),
